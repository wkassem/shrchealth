...
function showError(mountId, message, error) {
  ...
  if (!mountNode) return;
  mountNode.innerHTML = `<div style="max-width:900px;margin:2rem auto;padding:1rem;border:1px solid #fecaca;background:#fff1f2;color:#991b1b;border-radius:12px;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;"><strong>${message}</strong><pre style="margin-top:0.75rem;white-space:pre-wrap;">${error ? String(error) : ''}</pre></div>`;
}
